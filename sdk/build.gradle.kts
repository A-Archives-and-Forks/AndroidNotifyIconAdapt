    ...
    plugins.withId(dokkaPluginId) {
        configure<DokkaExtension> {
            ...
            pluginsConfiguration.withType<DokkaHtmlPluginParameters>().configureEach {
                footerMessage.set("ANIP | Apache-2.0 License | Copyright (C) 2019 HighCapable")
            }
        }
        ...
        tasks.register("publishKDoc") {
            ...
            doLast {
                val docsDir = rootProject.projectDir.parentFile
                    .resolve("docs-source")
                    .resolve("dist")
                    .resolve("KDoc")
...
fun libraryProjects(action: Action<in Project>) {
    val libraries = listOf(Libraries.ANIP_SDK)
    allprojects { if (libraries.contains(name)) action.execute(this) }
}

object Libraries {
    const val ANIP_SDK = "anip-sdk"
}